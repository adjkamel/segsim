...
def func_eval(dataset, recog_path, file_list, ground_truth_path, mapping_file):
...
def main(model_type):
    ...
    file_list = args.dataset+"/splits/test.split"+args.split+".bundle"
    ...
    if args.split == 0:
        for split in range(1, cnt_split_dict[args.dataset] + 1):
            recog_path = "./{}/".format('results')+args.dataset+"/split_{}".format(split)+"/"
        ...
        split = args.split
        acc_all, edit_all, f1s_all = func_eval(args.dataset, recog_path, file_list, ground_truth_path, mapping_file)
