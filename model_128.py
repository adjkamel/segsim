...
class Encoder(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate, att_type, alpha):
        ...
        self.stage = SingleStageModel(4, 128, 128, 128)
    ...
    def forward(self, x, mask):
        ...
        res=feature
        feature=self.stage(feature, mask)
        feature=feature+res
...
class Trainer:
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate):
        self.model = Encoder128( num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate)
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, batch_gen_tst=None):
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                optimizer.zero_grad()
                ps = self.model(batch_input, mask)[0].squeeze(dim=0)
    ...
    def test(self, batch_gen_tst, epoch):
        ...
        with torch.no_grad():
            while batch_gen_tst.has_next():
                ...
                batch_input, batch_target, mask = batch_input.to(device), batch_target.to(device), mask.to(device)
                p = self.model(batch_input, mask)[0].squeeze(dim=0).to(device)
                p = [p]
...
#                 print(vid)
                ...
                predictions = self.model(input_x, torch.ones(input_x.size(), device=device))[0].squeeze(dim=0)
